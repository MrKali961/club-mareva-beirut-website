...
export default function CancelClient({ token }: { token: string }) {
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      const res = await fetch(`${apiBase}/events/cancel/${token}`, {
        method: 'POST',
      });
      ...
      >
        {state.status === 'confirm' && (
          <>
            ...
            <h1 className="font-playfair text-3xl text-cream mb-3 tracking-wider">Cancel Registration?</h1>
            <p className="font-playfair text-cream/70 text-sm mb-8">
              Are you sure you want to cancel your event registration? This action will release your reserved spot.
            </p>
        ...
        {state.status === 'loading' && (
          <>
            ...
            <p className="font-playfair text-cream/70 text-sm">Cancelling your registration...</p>
          </>
        )}
        ...
        {state.status === 'success' && (
          <>
            ...
            <h1 className="font-playfair text-3xl text-cream mb-3 tracking-wider">Registration Cancelled</h1>
            <p className="font-playfair text-cream/70 text-sm mb-6">
              Your registration for <span className="text-gold">{state.eventTitle}</span> has been cancelled successfully.
            </p>
